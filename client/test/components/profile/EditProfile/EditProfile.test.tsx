...
describe('<EditProfile />', () => {
  ...
  test('renders with user data and handles submission', async () => {
    ...
    await waitFor(() => {
      expect(mockUpdateUser).toHaveBeenCalledWith('user_1', expect.objectContaining({
        name: 'Jane Doe',
  ...
  test('handles file upload', async () => {
    ...
    const fileInput = screen.getByRole('button', { name: /Change Photo/i }).previousSibling as HTMLInputElement;
